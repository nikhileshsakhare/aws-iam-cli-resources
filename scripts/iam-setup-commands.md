...
## 1. Configure AWS CLI for admin user: aws configure

Follow prompts for AWS Access Key ID, Secret Access Key, Region, and output format

## 2. Create IAM user (admin for IAM operations): aws iam create-user --user-name iam-admin
...
## 3. Create an additional IAM user: aws iam create-user --user-name ram

List users to verify: aws iam list-users

## 4. Create IAM group: aws iam create-group --group-name devops

List groups to verify: aws iam list-groups
...
List customer-managed policies in the account: aws iam list-policies --scope Local
...
List roles to verify: aws iam list-roles
